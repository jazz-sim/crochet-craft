...
function stringTokenLUT(input: string): ParsedToken {
    ...
    const notImplementedLUT = new Set<string>(['sp', 'hdc', 'slst']);
    ...
    // Misc. aliases
    const repeat = new Set<string>(['rep', 'repeat']);
    if (repeat.has(input)) {
        return { type: TokenType.Repeat, value: 'repeat' };
    }

    const times = new Set<string>(['time', 'times']);
    if (times.has(input)) {
